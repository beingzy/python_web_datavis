...
function makeGraph(error, projectJson, statJson) {
	// data processing
	...
	var povertyLevelDim = ndx.dimension(function(d) { return d["school_state"]; });
	var sateDim = ndx.dimension(function(d) { return d["school_state"]; });
    ...
    // pass neccessary parameters for each chart
    numberProjectsND
    ...
    timeChart
    	...
    	.x(d3.time.scale().domaion([minDate, maxDate]))
    	.elasticY(true)
    	.xAxisLabel("Year")
    	.yAxis().ticks(4);
